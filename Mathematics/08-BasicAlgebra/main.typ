...
#定义条目("环", "Ring")[
  // TODO: $(R, +, dot)$, 其中 $(R, +)$ 是阿贝尔群, $(R, dot)$ 是幺半群, 且乘法对加法满足左右分配律。
]

#注条目("", "")[
  - 本笔记“环”默认指“含幺环”(unital ring); 不含幺的统称“rng”/“伪环”不在主线讨论。
  - 通常省去乘号 $dot$, 把 $a dot b$ 简写为 $a b$。
]

#定义条目("交换环", "Commutative Ring")[
  // TODO: 乘法满足交换律的环。
]

#定义条目("零环", "Zero Ring")[
  // TODO: 只含一个元素的环 $\{0\}$, 加法与乘法均平凡, $0 = 1$。
]
...
#定义条目("单位 / 可逆元", "Unit")[
  // TODO: $u in R$ 称为单位, 当且仅当存在 $v in R$ 使得 $u v = v u = 1$。
]

#定义条目("单位群", "Group of Units")[
  // TODO: $R$ 中所有单位构成的乘法群, 记作 $R^times$。
]

#定义条目("零因子", "Zero Divisor")[
  // TODO: 非零元 $a in R$ 称为左(右)零因子, 若存在非零 $b in R$ 使 $a b = 0$ ($b a = 0$)。
]

#定义条目("幂零元", "Nilpotent")[
  // TODO: $a in R$ 称为幂零, 若存在 $n in NN$ 使得 $a^n = 0$。
]
...
#定义条目("整环", "Integral Domain")[
  // TODO: 无零因子的非零交换环。
]

#定义条目("除环 / 体", "Division Ring")[
  // TODO: 非零元全可逆的环 (不要求交换)。
]
...
#定义条目("环的特征", "Characteristic")[
  // TODO: 满足 $n dot 1 = 0$ 的最小正整数 $n$; 若不存在则规定 $"char"(R) = 0$。
]
...
#定义条目("子环", "Subring")[
  // TODO: $S subset.eq R$ 在加法、乘法、含幺元下封闭。
]
...
#定义条目("右理想", "Right Ideal", isExtension: true)[
  // TODO: $I subset.eq R$ 是加法子群, 且 $I R subset.eq I$。
]

#定义条目("双边理想 / 理想", "Two-sided Ideal", isExtension: true)[
  // TODO: 同时为左、右理想; 在交换环中三者等同, 简称理想。
]

#定义条目("真理想", "Proper Ideal")[
  // TODO: 不等于整个 $R$ 的理想。
]

#定义条目("由集合生成的理想", "Ideal Generated by a Set")[
  // TODO: 包含给定子集 $S subset.eq R$ 的最小理想, 记作 $angle.l S angle.r$。
]

#定义条目("主理想", "Principal Ideal")[
  // TODO: 由单个元素生成的理想 $(a) = R a$ (交换环情形)。
]

#定义条目("素理想", "Prime Ideal")[
  // TODO: 真理想 $P$, 满足 $a b in P ==> a in P or b in P$。
]

#定义条目("极大理想", "Maximal Ideal")[
  // TODO: 真理想 $M$, 不被任何真理想真包含。
]

#性质条目("极大理想是素理想", "")[
  // TODO: 在含幺交换环中, $M$ 极大 $==> M$ 素。
]


== 理想运算

#定义条目("理想的和", "Sum of Ideals")[
  // TODO: $I + J = \{ a + b | a in I, b in J \}$, 仍是理想。
]

#定义条目("理想的积", "Product of Ideals")[
  // TODO: $I J = angle.l \{ a b | a in I, b in J \} angle.r$, 仍是理想。
]

#定义条目("理想的交", "Intersection of Ideals")[
  // TODO: $I sect J$ 是理想。
]
...
#构造条目("商环", "Quotient Ring")[
  // TODO: 给定理想 $I$, 在 $R$ 上定义等价关系 $a tilde b iff a - b in I$, 得商集 $R \/ I$, 以陪集运算 $(a + I) + (b + I) = (a + b) + I$, $(a + I)(b + I) = (a b) + I$ 配成环。
]

#定义条目("商环", "Quotient Ring")[
  // TODO: 由理想 $I$ 决定的商环 $R \/ I$。
]

#性质条目("商环是域当且仅当理想极大", "")[
  // TODO: $R$ 含幺交换环, $M lt.tri R$: $R\/M$ 是域 $iff M$ 极大。
]

#性质条目("商环是整环当且仅当理想素", "")[
  // TODO: $R$ 含幺交换环, $P lt.tri R$: $R\/P$ 是整环 $iff P$ 素。
]


== 环同态

#定义条目("环同态", "Ring Homomorphism")[
  // TODO: $f : R -> S$ 保加法、保乘法、保单位元。
]

#定义条目("环同态的核", "Kernel")[
  // TODO: $ker(f) = \{ a in R | f(a) = 0 \}$, 是 $R$ 的理想。
]

#定义条目("环同态的像", "Image", isExtension: true)[
  // TODO: $"im"(f) = \{ f(a) | a in R \}$, 是 $S$ 的子环。
]

#定义条目("环同构", "Ring Isomorphism")[
  // TODO: 双射环同态。
]
...
#定义条目("多项式环", "Polynomial Ring")[
  // TODO: $R[x]$ 上以 $x$ 为不定元的形式多项式集合, 配以多项式加法与乘法。
]

#定义条目("多项式的次数", "Degree of a Polynomial")[
  // TODO: $deg(f)$ 为最高非零项的指数; 规定 $deg(0) = -infinity$。
]

#定义条目("首一多项式", "Monic Polynomial")[
  // TODO: 最高次项系数为 $1$ 的多项式。
]
...
#定义条目("不可约多项式", "Irreducible Polynomial")[
...
#定义条目("整除", "Divides")[
  // TODO: $a | b iff exists c, b = a c$。
]

#定义条目("素元", "Prime Element")[
  // TODO: $p$ 非零非单位, 且 $p | a b ==> p | a or p | b$。
]

#定义条目("不可约元", "Irreducible Element")[
...
#定义条目("欧几里得整环", "Euclidean Domain")[
  // TODO: 整环 $R$ 配以欧几里得范数 $delta : R \\ \{0\} -> NN$, 满足带余除法。
]

#定义条目("主理想整环", "Principal Ideal Domain")[
  // TODO: 整环, 其每个理想都是主理想。
]

#定义条目("唯一分解整环", "Unique Factorization Domain")[
...
#定义条目("环范畴", "Category of Rings")[
  // TODO: 范畴 $bold("Ring")$, 对象为含幺环, 态射为环同态。
]
...
#定义条目("环直积", "Direct Product of Rings")[
  // TODO: $R times S$ 上逐分量加乘, 为 $bold("Ring")$ 中的积。
]

#性质条目("环范畴有积无余积 (交换环情形不同)", "")[
  // TODO: $bold("Ring")$ 有全部小积, 余积非平凡(交换情形为张量积 $R times.circle_ZZ S$)。
]
